...
**Cirno_TV & Friends** is a collaborative effort between [DaemonTsun](https://bsky.app/profile/daemontsun.bsky.social) & [NopeTooFast](https://bsky.app/profile/nopetoofast.bsky.expert), which both retextures existing Jokers and adds some new ones, as well as various other changes such as some Joker names & descriptions, retextures of some enhancers & all blinds, and more!
...
- You have finished installing Cirno & Friends. *Changes may not immediately appear* as they need to be enabled in both the Steamodded mod menu (Specifically in this mod's config) and in Malverk's textures menu for them to appear in-game.
- *IMPORTANT*: For whatever reason, with this mod active, enabling the default textures in Malverk and then trying to interact with its card afterwards causes the game to error. Disabling the default card either requires removal of this mod or removal of the '"default",' line in Malverk's config file under ["selected"], Which itself is generally accessible by going up one level from the .../Balatro/mods/ folder, into the /config folder that should be under that /Balatro directory.
